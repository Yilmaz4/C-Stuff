...
GLfloat vertices[] = {
	...
	+0.0f / 1, -0.5f * float(sqrt(3)) / 3, 0.0f, // Inner down
};
...
int opengl_main(int argc, char* argv[]) {
	...
	if (!window) {
		boxer::show("OpenGL window creation was unsuccessful", "Could not create window", boxer::Style::Info, boxer::Buttons::OK);
		glfwTerminate();
		return -1;
	}
